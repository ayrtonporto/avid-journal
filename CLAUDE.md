...
```
.tex → src/parser/ → src/formalization/ (Claude Code + Lean 4) → src/novelty/ (3 dimensions) → verdict
```
...
2. **`src/novelty/`** — módulo unificado de novedad. Contiene el orquestador, las 3 dimensiones, y todos los sub-módulos (mathlib_checker, llm_judge, arxiv_search, etc.).
...
├── mathlib_checker.py    ← Leandex v2 API format
├── llm_judge.py          ← DeepSeek V4 Flash vía OpenCode Go
├── arxiv_search.py       ← arXiv + Semantic Scholar
├── _cache.py             ← Cache compartido
├── run_eval_full.py      ← ✅ Eval script con checkpointing
├── prewarm_premise_cache.py ← Pre-calentamiento caché D3
├── validate_d3.py        ← Validación D3
├── d3_extraction_map.yaml ← Mapeo IDs → archivos .lean para D3
└── d3_filter_blacklist.yaml ← Filtros D3
src/formalization/
└── batch_formalize_informal.py ← Formalización batch de matches informales
lean_project/
