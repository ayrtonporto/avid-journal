...
def findLean (mod : Name) : IO FilePath := do
  ...
  -- We relax this: some dependency sources may not be available (e.g. core Lean).
  -- Return the computed path anyway; callers should handle missing files gracefully.
  if !(← path.pathExists) then
    IO.eprintln s!"WARNING: Dependency source not found: {path}"
  return path
