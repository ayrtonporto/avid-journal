...
def _resolve_executable(args: List[str]) -> List[str]:
    """AViD: Windows compatibility fix.

    On Windows, subprocess.Popen does not consult PATHEXT to find .cmd / .bat
    wrappers (which is how `claude` is installed via npm). Without resolving
    the absolute path, Popen raises [WinError 2].

    This helper rewrites args[0] to the absolute path returned by shutil.which
    if (a) it is just a bare name (no path separator) and (b) shutil.which
    finds a match. Otherwise args is returned unchanged.
    """
    if not args:
        return args
    cmd = args[0]
    if os.sep in cmd or "/" in cmd:
        return args  # already a path
...
# AViD: also force UTF-8 with errors="backslashreplace" so prints of Lean
# output (which contains symbols like exists, forall, leq, etc.) do NOT
# crash on Windows consoles using cp1252 / cp850.
try:
...
# AViD: detect Anthropic rate-limit / quota exhausted messages. When Claude's
# subscription has been exhausted, the response body becomes a short message
# like "You've hit your limit · resets 3:40pm (America/Buenos_Aires)" instead
# of a real assistant response. We need to distinguish this from genuine
...
def run_claude_once(
    ...
    Args:
        ...
        prompt: Optional prompt to send via stdin instead of as a command-line
                argument. AViD: required on Windows when the prompt exceeds the
                cmd.exe command-line length limit (~8191 chars). When provided,
                the prompt is NOT appended to args; claude reads it from stdin.
    ...
    # "La linea de comandos es demasiado larga" on Windows.
    ...
    with open(json_save_path, "w", encoding="utf-8") as stdout_target:
        proc = subprocess.Popen(
            ...
            text=True,
            encoding="utf-8" if prompt is not None else None,
            env=env or None,
        ...
        if prompt is not None:
            try:
                ...
                print(f"[warn] Could not write prompt to claude stdin: {e}")
        proc.wait()
    ...
    if last_line:
        try:
            ...
            if parsed.get("type") == "result":
                ...
                # AViD: Anthropic rate-limit detection takes precedence over
                # a missing END_REASON. The result body becomes the limit
                # notice itself, with no END_REASON tag.
                if reason is None and PAT_RATE_LIMIT.search(result_text):
                    reason = "RATE_LIMITED"
                    print(
                        "[error] Anthropic rate limit / quota exhausted "
                        "detected in Claude's response. Aborting session."
                    )
...
def run_claude_session(
    ...
    reason, returncode, claude_result = run_claude_once(
        base, env=env, cwd=cwd, json_save_path=_get_json_save_path(1),
        prompt=prompt,
    )
    ...
    # AViD: rate-limit detection short-circuits everything. We exit the
    # session immediately with a special end_reason that the caller (the
    # AViD orchestrator) interprets as "abort the whole paper run".
    if reason == "RATE_LIMITED":
        return "RATE_LIMITED", 1, round_results
    ...
    while reason == "LIMIT" or reason is None or reason == "COMPLETE" or reason == "SELECTED_TARGET_COMPLETE":
        ...
        if reason == "COMPLETE":
            ...
            if on_complete:
                if on_complete():
                    ...
                    round_start = time.time()
                    reason, returncode, claude_result = run_claude_once(
                        base, env=env, cwd=cwd, json_save_path=_get_json_save_path(rounds),
                        prompt=prompt,
                    )
                    round_duration = time.time() - round_start
                    last_claude_result = claude_result
        ...
        if reason is None:
            ...
            reason, returncode, claude_result = run_claude_once(
                cmd, env=env, cwd=cwd, json_save_path=_get_json_save_path(rounds),
                prompt="continue",
            )
        round_duration = time.time() - round_start
        last_claude_result = claude_result
        ...
        # AViD: rate-limit short-circuit (mid-loop case). If Claude's
        # subscription gets exhausted partway through a session, abort
        # immediately so the orchestrator can surface the issue.
        if reason == "RATE_LIMITED":
            return "RATE_LIMITED", rounds, round_results
...
def run_task(task: TaskMetadata) -> TaskResult:
    ...
    try:
        ...
        def on_complete_callback() -> bool:
            ...
            if task.allow_sorry:
                errors = [(f, e, s, out, err) for f, e, s, out, err in results if e]
                print(f"[info] allow_sorry=True, ignoring sorry warnings")
            else:
                errors = [(f, e, s, out, err) for f, e, s, out, err in results if e or s]

            if errors:
                print(f"\n[error] {len(errors)} files have errors{'' if task.allow_sorry else '/sorry'}:")
                for f, e, s, out, err in errors:
                    print(f"  - {f}")
                    print(f"    has_error={e} has_sorry={s}")
                    if out:
                        snippet = out.strip().splitlines()
                        if snippet:
                            print(f"    stdout: {snippet[0][:200]}")
                            if len(snippet) > 1:
                                print(f"            ({len(snippet)} more lines)")
                    if err:
                        snippet = err.strip().splitlines()
                        if snippet:
                            print(f"    stderr: {snippet[0][:200]}")
                            if len(snippet) > 1:
                                print(f"            ({len(snippet)} more lines)")
                return False
        ...
        # AViD: si la sesion termino por rate-limit, NO hacemos final verify
        # ni sobreescribimos end_reason (sino que un stub vacio que compila
        # haria que reportemos success=True, ocultando la causa real).
        if end_reason == "RATE_LIMITED":
            print(
                "[info] Rate limit detected: skipping final verification and "
                "preserving end_reason=RATE_LIMITED for the orchestrator."
            )
            success = False
            # Saltar todo el bloque de "final check" y SafeVerify
            check_path = None
            lean_files = []
            mcp_stats = None
            # Build TaskResult directly y returnarlo
            ...
            result = TaskResult(
                ...
                end_time=end_time,
                error_message="Anthropic rate limit / quota exhausted",
                mcp_stats=None,
                round_results=round_results,
                statement_changed=statement_changed,
                safe_verify_result=None,
            )
